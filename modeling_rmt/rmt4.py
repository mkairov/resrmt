...
class MemoryCell(torch.nn.Module):
    def __init__(self, base_model, num_mem_tokens, init_inner_lr=1.0, learn_lr=False, inner_steps=10, inner_optim='sgd', **kwargs):
        ...
        self.momentum_mode = kwargs.get('momentum_mode', None)
        self.momentum = None
    ...
    def _sgd_step(self, p, g, beta=0.95, nesterov=True):
        g = g.reshape_as(p)
        ...
        if self.momentum_mode is not None:
            if self.momentum is None:
                self.momentum = torch.zeros_like(p, memory_format=torch.preserve_format)
            self.momentum.lerp_(g, 1 - beta)
            update = g.lerp_(self.momentum, beta) if nesterov else self.momentum
    ...
    def _muon_step(self, p, g, beta=0.95, nesterov=True, ns_steps=5):
        if self.inner_clip_value is not None:
            # simple element-wise clamp
            g = torch.clamp(g, -self.inner_clip_value, self.inner_clip_value)
    ...
    def inner_loop(self, input_ids, **seg_kwargs):
        ...
        for _ in range(self.inner_steps):
            ...
            if self.inner_optim == 'sgd':
                mem_live = self._sgd_step(mem_live, g)
            elif self.inner_optim == 'muon':
                mem_live = self._muon_step(mem_live, g)
            else:
...
class RecurrentWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids, labels=None, labels_mask=None, inputs_embeds=None, attention_mask=None, output_attentions=None, output_hidden_states=None):
        ...
        self.memory_cell.momentum = None
