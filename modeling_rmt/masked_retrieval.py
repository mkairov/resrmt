...
class MemoryCell(torch.nn.Module):
    def __init__(self, base_model, num_mem_tokens, init_inner_lr=1.0, learn_lr=False, inner_steps=10, inner_optim='sgd', **kwargs):
        ...
        self.max_n_segments = kwargs.get('max_n_segments', 2) - 1
    ...
    def inner_loop(self, input_ids, **seg_kwargs):
        ...
        # mem_live = inputs_embeds[:, (len(self.mem_list) - 1) * self.num_mem_tokens:len(self.mem_list) * self.num_mem_tokens]
        mem_live = inputs_embeds[:, self.num_mem_tokens * (self.max_n_segments - 1):self.num_mem_tokens * self.max_n_segments]
        mem_live = mem_live.requires_grad_(True)
        orig_embeds = inputs_embeds[:, self.num_mem_tokens * self.max_n_segments:]

        device = inputs_embeds.device
        inner_loop_stats = {'inner_grad_norm_mean': torch.tensor(0.0, device=device)}
        
        for _ in range(self.inner_steps):
            ctx_emb = torch.cat(self.mem_list + [mem_live] * (self.max_n_segments - len(self.mem_list)) + [orig_embeds], dim=1)
            seg_kwargs['inputs_embeds'] = ctx_emb
            out = self.model(**seg_kwargs)

            if self.use_write_head:
                h = out.hidden_states[-1]
                h = h[:, self.num_mem_tokens * self.max_n_segments:, :]
                logits = self.write_head(h)
                del h
            else:
                logits = out.logits
                logits = logits[:, self.num_mem_tokens * self.max_n_segments:, :]
    ...
    def forward(self, input_ids, memory_state=None, is_last_segment=False, segment_num=None, **kwargs):
        if memory_state is None:
            memory_state = self.set_memory(input_ids.shape)
        
        seg_kwargs = self.process_input(input_ids, memory_state, is_last_segment=is_last_segment, segment_num=segment_num, **kwargs)
    ...
    def generate(self, input_ids, memory_state, attention_mask=None, segment_num=None, **generate_kwargs):
        if memory_state is None:
            memory_state = self.set_memory(input_ids.shape)

        seg_kwargs = self.process_input(input_ids, memory_state, attention_mask=attention_mask, is_last_segment=True, segment_num=segment_num)
        out = self.model.generate(inputs_embeds=seg_kwargs['inputs_embeds'], attention_mask=seg_kwargs['attention_mask'], **generate_kwargs)
        return out

    def process_input(self, input_ids, memory_state, is_last_segment, segment_num, **kwargs):
        ...
        if self.num_mem_tokens > 0:
            inputs_embeds = torch.cat(self.mem_list + [memory_state] * (self.max_n_segments - len(self.mem_list)) + [inputs_embeds], dim=1)

        seg_kwargs['input_ids'] = None
        seg_kwargs['inputs_embeds'] = inputs_embeds
        if kwargs.get('attention_mask') is not None:
            seg_kwargs['attention_mask'] = self.pad_attention_mask(kwargs['attention_mask'], inputs_embeds.shape, is_last_segment, segment_num)
        seg_kwargs['output_hidden_states'] = True
        return seg_kwargs
    
    def pad_attention_mask(self, attention_mask, shape, is_last_segment, segment_num):
        ...
        #     mask[:, self.num_mem_tokens * segment_num:self.num_mem_tokens * (segment_num + 1)] = 1
        return mask
    
    def process_output(self, model_outputs, inner_stats, **kwargs):
        if self.num_mem_tokens not in {0, None}:
            out = CausalLMOutputWithCrossAttentions()
            logit_start = self.num_mem_tokens * self.max_n_segments

            out['logits'] = model_outputs.logits[:, logit_start:]

            if kwargs.get('output_hidden_states'):
                out['hidden_states'] = [lh[:, logit_start:] for lh in model_outputs.hidden_states]
...
class RecurrentWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids, labels=None, labels_mask=None, inputs_embeds=None, attention_mask=None, output_attentions=None, output_hidden_states=None):
        ...
        for seg_num, segment in enumerate(segmented):
            ...
            cell_out, memory_state = self.memory_cell(**segment, memory_state=memory_state, is_last_segment=is_last_segment, segment_num=seg_num, output_hidden_states=True)
    ...
    def generate(self, input_ids, attention_mask=None, **generate_kwargs):
        ...
        for seg_num, segment in enumerate(segmented[:-1]):
            _, memory_state = self.memory_cell(**segment, memory_state=memory_state, is_last_segment=False, segment_num=seg_num, output_hidden_states=True)

        final_segment = segmented[-1]
        out = self.memory_cell.generate(**final_segment, memory_state=memory_state, segment_num=len(segmented)-1, **generate_kwargs)
