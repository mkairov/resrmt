...
class AssociativeLayerWrapper(torch.nn.Module):
    ...
    def __init__(self, layer, d_model, num_mem_tokens, d_mem, correction=True, info=None, **kwargs) -> None:
        super().__init__()
        self.info = info
        self.seg_num = 0
    ...
    def associate(self, hidden_states):
        ...
        self.W_mem = self.W_mem.to(hidden_states.device).to(hidden_states.dtype)
        self.z = self.z.to(hidden_states.device).to(hidden_states.dtype)
        
        mq = self.phi(self.W_mq(hidden_states)).to(hidden_states.dtype) # (bsz, seq_len, 2d_mem * nu)
...
class AssociativeMemoryCell(torch.nn.Module):
    def __init__(self, base_model, num_mem_tokens, d_mem, layers_attr: str = 'transformer.h', wrap_pos=False, correction=True, **kwargs):
        super().__init__()
    ...
    def create_memory(self, num_mem_tokens):
        ...
        memory_weights = torch.randn((num_mem_tokens, memory_dim)) * embeddings.weight.data.std().cpu().item()
    ...
    def process_input(self, input_ids, **kwargs):
        ...
        if self.wrap_pos:
            ...
            ordinary_pos = torch.arange(0, input_ids.size(1), dtype=inputs_embeds.dtype, device=input_ids.device)
            write_pos = torch.arange(num_pos_embs - self.num_mem_tokens, num_pos_embs, dtype=inputs_embeds.dtype, device=input_ids.device)
